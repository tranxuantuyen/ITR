...
from .modeling.transformer_decoder.itr_decoder_model import ItrSpatialTemporal
...
class ITR(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.nlp2.add_pipe("benepar", config={"model": "benepar_en3"})
        self.tokenizer = RobertaTokenizerFast.from_pretrained("roberta-base")
    ...
    def from_config(cls, cfg):
        ...
        vita_module = ItrSpatialTemporal(cfg=cfg, in_channels=hidden_dim, aux_loss=vita_deep_supervision)
    ...
    def train_model(self, batched_inputs):
        ...
        noun_phrase, verb_phrase = self._decompose_sentence(batched_inputs[0]['sentence'])
        ...
        if len(verb_phrase) > 0:
            noun_phrase = [noun.unsqueeze(0).to(self.device) for noun in noun_phrase]
            verb_phrase = [verb.unsqueeze(0).to(self.device) for verb in verb_phrase]
            noun_feat = [lang_feat_fusion[noun.bool()] for noun in noun_phrase]
            verb_feat = [lang_feat_fusion[verb.bool()] for verb in verb_phrase]
        else:
            noun_feat = [static_feat]
            verb_feat = [motion_feat]
        itr_feat = (noun_feat, verb_feat)
        ...
        return losses
    def is_VP(self, pattern, text):
        if text.startswith("(VP") == False:
            return False
        occurrences = re.findall(pattern, text)
        return len(occurrences) == 1
    
    def _decompose_sentence(self, sentence_raw):
        encoded_input = self.tokenizer(sentence_raw, return_tensors="pt", add_special_tokens=True, return_offsets_mapping=True)
        ...
        with autocast(enabled=False):
            doc = self.nlp2(sentence_raw)
            sent = list(doc.sents)[0]
        save = []
        subject = list(doc.sents)[0]
        while True:
            if len(list(subject._.children)) == 0:
                break
            if len(list(subject._.children)[0]._.labels) == 0:
                break
            if list(subject._.children)[0]._.labels[0] == "NP":
                subject = list(subject._.children)[0]
            else:
                break
        for element in list(sent._.constituents):
            if self.is_VP("\(VP", element._.parse_string):
                save.append(element)
        noun_phrase = torch.zeros(40)
        verb_phrase = [torch.zeros(40) for _ in range(len(save))]
        for index, (rt, offset) in enumerate(zip(roberta_tokens, roberta_offsets)):
            start, end = offset
            if rt != '<s>' and rt != '</s>':
                for token in doc:
                    if token.idx <= start and (token.idx + len(token.text)) >= end:
                        if token.text in subject.text:
                            try:
                                noun_phrase[index] = 1
                            except:
                                break
                            break
                        try:
                            for v_id, verb in enumerate(save):
                                if token.text in verb.text:
                                    verb_phrase[v_id][index] = 1
                        except:
                            break
        num_phrase = len(verb_phrase)
        noun_phrase = [noun_phrase for _ in range(num_phrase)]
    ...
    def inference(self, batched_inputs):
        ...
        noun_phrase, verb_phrase = self._decompose_sentence(batched_inputs['sentence'])
        motion_map = motion_map.unsqueeze(0).to(self.device)
        static_map = static_map.unsqueeze(0).to(self.device)
